  ...
  <div
      ...
      <div
        ...
        <ul class="list-unstyled flex flex-col gap-1.5">
          ...
          <li>
            <button
              class="w-full text-left text-white grid grid-cols-[24px_1fr_24px] items-center gap-1 xl:gap-3 px-1 xl:px-4 py-3 hover:bg-white/10 rounded-lg transition-all duration-200 group cursor-pointer"
              @click="toggleSubmenu('accesoriosSubmenu')">
              <i class="fa-solid fa-gears text-center w-5 text-white group-hover:text-white"></i>
              <span
                class="whitespace-nowrap group-hover:translate-x-1 transition-transform duration-200">Accesorios</span>
              <svg class="ml-auto w-4 h-4 transition-transform duration-300"
                :class="{ 'rotate-180': openSubmenus.accesoriosSubmenu }" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            ...
            <ul class="relative overflow-hidden transition-all duration-300"
              :style="{ maxHeight: openSubmenus.accesoriosSubmenu ? accesoriosSubmenuHeight + 'px' : '0px' }"
              ref="accesoriosSubmenu">
              <div class="absolute left-6 w-0.5 h-33 bg-gray-200 opacity-80 rounded-sm mr-3"></div>
              <!-- Categorías -->
              <li>
                <AppLink :href="route('accesorioscategorias.dashboard')"
                  class="block pl-10 py-2 text-gray-200 hover:text-white rounded-lg transition-all duration-200 my-1 cursor-pointer">
                  <span class="opacity-80 hover:opacity-100 transition-opacity flex items-center">
                    <i class="fa-solid fa-layer-group w-5 mr-2 text-sm"></i>
                    Categorías
              ...
              <li>
                <AppLink :href="route('accesoriosproductos.dashboard')"
                  class="block pl-10 py-2 text-gray-200 hover:text-white rounded-lg transition-all duration-200 my-1 cursor-pointer">
                  <span class="opacity-80 hover:opacity-100 transition-opacity flex items-center">
                    <i class="fa-solid fa-box w-5 mr-2 text-sm"></i>
                    Accesorios
    ...
    <div class="mt-auto">
      ...
      <p class="text-gray-300 text-xs text-center px-3">
        <span class="opacity-70 hover:opacity-100 transition-opacity duration-200">@2025 Cecconi - by Osole</span>
      </p>
    </div>
  </div>
</template>
  ...
  data() {
    return {
      ...
      openSubmenus: {
        ...
        accesoriosSubmenu: false,
      },
      homeSubmenuHeight: 0,
      productoSubmenuHeight: 0,
      accesoriosSubmenuHeight: 0,
    };
  },
  methods: {
    async toggleSubmenu(submenuId) {
      ...
      if (submenuId === 'accesoriosSubmenu' && this.$refs.accesoriosSubmenu) {
        this.accesoriosSubmenuHeight = this.$refs.accesoriosSubmenu.scrollHeight;
      }
    }
  },
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      if (this.$refs.accesoriosSubmenu) {
        this.accesoriosSubmenuHeight = this.$refs.accesoriosSubmenu.scrollHeight;
      }
    });
